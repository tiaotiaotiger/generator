...
import org.mybatis.generator.config.JavaParamConfiguration;
...
public class MyBatisGeneratorConfigurationParser {
	...
	private void parseContext(Configuration configuration, Node node) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			}else if("javaParam".equals(childNode.getNodeName())) {
	...
	private void parseJavaParam(Context context, Node node) {
		...
		Properties attributes = parseAttributes(node);
		String genericDao = attributes.getProperty("genericDao"); //$NON-NLS-1$
		String genericService = attributes.getProperty("genericService"); //$NON-NLS-1$
		String buildType = attributes.getProperty("buildType"); //$NON-NLS-1$
		String mapperSuffix = attributes.getProperty("mapperSuffix"); //$NON-NLS-1$

		javaParamConfiguration.setGenericDao(genericDao);
		javaParamConfiguration.setGenericService(genericService);
		javaParamConfiguration.setBuildType(buildType);
		javaParamConfiguration.setMapperSuffix(mapperSuffix);
